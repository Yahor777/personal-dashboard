  ...
  async chat(messages: Array<{ role: string; content: string }>): Promise<AIResponse> {
    return (
      (await errorHandler.withErrorHandling(
        ...
        "AI Chat Service",
        { showToast: false, retries: 0 }
      )) || {
        content: "Произошла ошибка при обращении к AI сервису",
        error: "SERVICE_ERROR",
      }
    );
  }
      ...
      new Promise<AIResponse>((_, reject) =>
        setTimeout(
          () =>
            reject(
              new Error(
                "Превышено время ожидания ответа (30 сек). Попробуйте снова или выберите другую модель."
              )
            ),
          this.timeout
        )
  ...
  private async callOpenRouter(messages: Array<{ role: string; content: string }>): Promise<AIResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return {
          content: `Ошибка OpenRouter (${response.status}): ${
...
export async function analyzeOLXListing(
  ...
  const categoryContext = listingData.category ? getCategoryContext(listingData.category) : "";
